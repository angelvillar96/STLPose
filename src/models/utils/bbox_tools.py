...
def bbox_filtering(bbox, labels, scores, filter=14):
    """
    ...
    for i, _ in enumerate(labels):
        ...
        for j, _ in enumerate(labels[i]):
            if(labels[i][j] == filter):
                print("hola")
                reshaped_bbox = [bbox[i][j][0], bbox[i][j][1], bbox[i][j][2], bbox[i][j][3]]
                cur_bbox.append(reshaped_bbox)
                # cur_bbox.append(bbox[i][j])
                cur_labels.append(labels[i][j])
                cur_scores.append(scores[i][j])
        # if(len(cur_bbox) == 0):
            # continue
