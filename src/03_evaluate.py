    ...
    def evaluate_model(self):
        ...
        with torch.no_grad():
            for i, (imgs, target, target_weight, metadata) in enumerate(tqdm(self.valid_loader)):
                ...
                keypoints, max_vals,\
                    coords = pose_parsing.get_final_preds_hrnet(output.cpu().numpy(),
                                                                centers, scales)

                # saving image results if save parameter is activated
                if(self.params.save == True):
                    poses, kpts =\
                        pose_parsing.create_pose_from_outputs(dets=output,
                                                              keypoint_thr=self.kpt_thr)
                    for n in range(len(poses)):
                        savepath = os.path.join(self.img_path,
                ...
                # TODO: Migrate the chunk below to some library function
...
if __name__ == "__main__":
    ...
    message = f"Initializing model evaluation."
    ...
    trainer = Evaluator(exp_path=exp_path, checkpoint=checkpoint,
                        dataset_name=dataset, params=params)
    trainer.setup_model_dataset()
    trainer.evaluate_model()
    logger.log_info(message=f"Evaluation finished successfully.")
