...
from lib.pose_parsing import get_max_preds_hrnet, create_pose_entries
...
class ArchDataExtractor:
    ...
    def load_models(self):
        ...
        if(self.checkpoint is not None):
            ...
            self.pose_model = load_checkpoint(
                    checkpoint_path,
    ...
    def extract_retrieval_dataset(self):
        ...
        """
        self.retrieval_db = {}
        rescale_dets = lambda dets: F.interpolate(dets, (256, 192), mode="bilinear", align_corners=True)
        ...
        for i, (imgs, _, _, metadata) in enumerate(tqdm(self.valid_loader)):
            ...
            # flipped forward pass: averages results of original and flipped image
            keypoint_dets = inference.forward_pass(
                    model=self.pose_model,
                    img=imgs,
...
if __name__ == "__main__":
    os.system("clear")
    exp_path, checkpoint, dataset, params = get_directory_argument(get_checkpoint=True, get_dataset=True)

    # initializing logger and logging the beggining of the experiment
    logger = Logger(exp_path)
    message = "Starting to extract ArchData retrieval dataset."
    logger.log_info(message=message, message_type="new_exp")

    extractor = ArchDataExtractor(
            exp_path=exp_path,
    ...
    logger.log_info(message="Dataset extracted successfully.")
